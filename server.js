...
app.get('/login', function (req, res) {
    ...
    if (response.address == "michael" && response.telephone == "bido") {
...
app.post('/upload', async (req, res, next) => {
    ...
    try {
        ...
        setTimeout(function() {
            if (confValue < 90.0 &&  imgLabel == "cliff") {
                res.sendFile(__dirname + "/public/final2.html");
            }
            else {
                res.sendFile(__dirname + "/public/index.html");
            }
...
async function checkFace() {
    ...
    const basePath = '../opencvcliff2/images_db';
    const subjectsPath = path.resolve(basePath, 'subjects');
    const nameMappings = ['cliff','conan', 'michael','dwayne'];
